...
// Golan's Pattern Master Functions:
  ...
  // clamped p5.js map function with terms reordered:
    ...
    // =============================================================================
       ...
       */
       p5.Filt = function(_fs) {
        //
        // this object implements filters based on Robert Bristow-Johnson's
        // Cookbook formulae for audio EQ biquad filter coefficients :
        // http://www.musicdsp.org/files/Audio-EQ-Cookbook.txt
        //
        // general 2p2z / biquad formula:
        // y[n] = (b0/a0)*x[n] + (b1/a0)*x[n-1] + (b2/a0)*x[n-2] - (a1/a0)*y[n-1] - (a2/a0)*y[n-2]
        //

        if(!_fs) _fs = 60.; // nominal web canvas framerate
        ...
        this.cw0 = cos(this.w0); // precompute
        this.sw0 = sin(this.w0); // precompute
        this.alpha = sin(this.w0)/(2*this.Q); // precompute
  ...
  //      this.Q = 1.0/(2*Math.sinh(log(2)/2*_bw*this.w0/this.sw0));
      ...
      p5.Filt.prototype.tick = function(_x) // STK style: sample-by-sample
      {
        // input
        this.xn = _x;
   ...
   */
   ...
   // FFT module pulled from dsp.js @corbanbrook
   p5.FastFourierTransform = function(bufferSize, sampleRate) {
     ...
     //FT.call(this, bufferSize, sampleRate);
   ...
   p5.FastFourierTransform.prototype.forward = function(buffer) {
     ...
     if (pow(2, k) !== this.bufferSize) { throw "Invalid buffer size, must be a power of 2."; }
     if (this.bufferSize !== buffer.length)  { throw "Supplied buffer is not the same size as defined FFT. FFT Size: " + bufferSize + " Buffer Size: " + buffer.length; }
  ...
  // Return signed distance from line Ax + By + C = 0 to point P. (h/t @golanlevin)
  p5.prototype.linetopoint = function(a, b, c, ptx, pty) {
    var lp = 0.0;
    var d = sqrt((a*a)+(b*b));
    if (d != 0.0) {
      lp = (a*ptx + b*pty + c)/d;
  ...
  p5.prototype.isPerpendicular = function(pt1x, pt1y, pt2x, pt2y, pt3x, pt3y)
  {
  ...
  // from Paul Bourke's Circle.cpp
  p5.prototype.calcCircleFrom3Points = function(pt1x, pt1y, pt2x, pt2y, pt3x, pt3y, m)
  {
    var yDelta_a = pt2y - pt1y;
    var xDelta_a = pt2x - pt1x;
    var yDelta_b = pt3y - pt2y;
