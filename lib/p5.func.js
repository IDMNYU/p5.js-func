...
(function (root, factory) {
  ...
  // =============================================================================
    ...
    p5.Gen.prototype.window = function(_x, _type, _args) {
      ...
      switch(_type) {
        ...
        case "slidingpoissonhann":
          if(!_args[0]) _args[0] = 0.5; // default center
          if(!_args[1]) _args[1] = 0.5; // default center
    ...
    p5.Gen.prototype.fillArray = function(_algo, _len, _args, _fourth) {
      ...
      if(_algo==='random') { // 4th argument is seed
        ...
        {
          if(_fourth) _p[i] = i/(_len-1)*10000.+_fourth;
          else _p[i] = 'm';
        }
      }
      else {
      ...
      _dest = this[_algo](_p, _args, _fourth);
      return(_dest);
    }
...
// Golan's Pattern Master Functions:
  ...
  p5.Ease.prototype.doubleEllipticSeat = function(_x, _a, _b)
  {
    if(!_a) _a = 0.25; // default
    if(!_b) _b = 0.75; // default
  ...
  // double-cubic seat
  p5.Ease.prototype.doubleCubicSeat = function(_x, _a, _b)
  {
    if(!_a) _a = 0.25; // default
    if(!_b) _b = 0.75; // default
  ...
  p5.Ease.prototype.doubleCubicSeatSimplified = function(_x, _a, _b)
  {
    if(!_a) _a = 0.25; // default
    if(!_b) _b = 0.75; // default
  ...
  p5.Ease.prototype.doubleExponentialSeat = function(_x, _a)
  {
    if(!_a) _a = 0.25; // default
  ...
  p5.Ease.prototype.doubleCircularSeat = function(_x, _a)
  {
    if(!_a) _a = 0.25; // default
  ...
  // double squircular seat
  p5.Ease.prototype.doubleSquircularSeat = function(_x, _a, _n)
  {
    if(!_a) _a = 0.25; // default
    if(!_n) _n = 3; // default
  ...
  // =============================================================================
     ...
     */
     p5.ArrayEval = function() {
      ...
      // easily becaused to execute arbitrary code.
      //
      ...
      // functionality to the p5.ArrayEval object, so, e.g. p5.js math functions
    ...
    p5.ArrayEval.prototype.eval = function(_evalstr, _l1) {
      ...
      var _dest;
      if(multi) _dest = this.createArray(_l1, _evalstr.length);
      else _dest = this.createArray(_l1);
      // string expansion:
      // u unwraps to 0-1
    ...
    p5.ArrayEval.prototype.eval2d = function(_evalstr, _l1, _l2) {
      ...
      var _dest;
      if(multi) _dest = this.createArray(_l1, _l2, _evalstr.length);
      else _dest = this.createArray(_l1, _l2);
      // string expansion:
      // u unwraps to 0-1
      // su unwraps to -1 to 1
      // cu unwraps to cell value (0 to dim-1)
    ...
    p5.ArrayEval.prototype.eval3d = function(_evalstr, _l1, _l2, _l3) {
      ...
      var _dest;
      if(multi) _dest = this.createArray(_l1, _l2, _l3, _evalstr.length);
      else _dest = this.createArray(_l1, _l2, _l3);
      // string expansion:
      // u unwraps to 0-1
      // su unwraps to -1 to 1
      // cu unwraps to cell value (0 to dim-1)
    ...
    // helper function to create n-dimensional arrays
   ...
   */
   p5.FastFourierTransform = function(_bufsize, _fs) {
     ...
     // data arrays for the magnitude / phas spectrum and the raw real/imaginary
   ...
   p5.FastFourierTransform.prototype.getBandFrequency = function(index) {
   ...
   p5.FastFourierTransform.prototype.calculateSpectrum = function() {
    ...
    p5.FastFourierTransform.prototype.forward = function(buffer) {

     var k = floor(log(this.bufferSize) / Math.LN2);

     if (pow(2, k) !== this.bufferSize) { throw "buffer size must be a power of 2."; }
     if (this.bufferSize !== buffer.length)  { throw "buffer is not the same size as defined FFT. FFT: " + bufferSize + " buffer: " + buffer.length; }
   ...
   p5.FastFourierTransform.prototype.inverse = function(real, imag) {
     ...
     for (i = 0; i < real.length; i++) {
       revReal[i] = real[this.reverseTable[i]];
       revImag[i] = imag[this.reverseTable[i]];
     }
     ...
     for (i = 0; i < this.bufferSize; i++) {
       buffer[i] = real[i] / this.bufferSize;
     }
  ...
  p5.prototype.sin4 = function(_x) {
    return(sin(_x)*sin(_x)*sin(_x)*sin(_x));
  }

  p5.prototype.sin8 = function(_x) {
