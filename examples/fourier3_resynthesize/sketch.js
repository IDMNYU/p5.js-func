...
var osc = new Array(10); // oscillator bank
...
function setup()
{
  ...
  {
    osc[i] = new Tone.Oscillator(440, "sine").toMaster().start();
    osc[i].volume.value = -100;
  }
...
function draw()
{
  ...
  hs+= 'PRESS A KEY TO FREEZE AND RESYNTHESIZE<br>';
...
function keyPressed()
{
  ...
  {
    osc[i].frequency.value = topten[i].freq;
    osc[i].volume.value = Tone.gainToDb(topten[i].mag);
  }
...
function keyReleased()
{
  for(var i = 0;i<topten.length;i++)
  {
    osc[i].volume.value = Tone.gainToDb(0);
  }
